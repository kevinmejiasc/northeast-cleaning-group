...
- `booking.js` — shared booking popup (loaded by every page). Any `#book` link or `[data-book]` element opens it; it collects contact + address, then a date and a 1-hour arrival window (8 AM–4 PM). Set the CRM webhook **once** via `BOOKING_WEBHOOK_URL` at the top of the file. Optional: `data-service` on a trigger pre-selects the service; `data-city` on `<body>` pre-fills the City field. The per-page inline `<script>` only holds the price calculator (`BASE`/`PER_BED`/`PER_BATH`, currently +30% over the original base).
